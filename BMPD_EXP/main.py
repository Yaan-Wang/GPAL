...
def main(local_rank, ngpus_per_node, hyp_param):
    ...
    for epoch in range(0, hyp_param.epochs):
        ...
        torch.distributed.barrier()
        print(f"Epoch {epoch}: mIoU: {mIoU}, mAcc: {mAcc}, mF1: {mF1}")

        # record results
        if writer is not None:
            # writer.add_scalar('Loss/train_loss', loss, epoch)##just the last interation loss of the epoch, not the average loss of the epoch
