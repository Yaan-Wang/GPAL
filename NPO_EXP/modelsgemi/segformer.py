...
class DE(nn.Module):
    def __init__(
        ...
        if "swin" in backbone:
            if backbone == "swin_tiny":
                pretrained_backbone_path = "../ckpts/swin_tiny_patch4_window7_224.pth"
                self.in_channels = [96, 192, 384, 768]
            elif backbone == "swin_small":
                pretrained_backbone_path = (
                    "../ckpts/swin_small_patch4_window7_224.pth"
                )
                self.in_channels = [96, 192, 384, 768]
            elif backbone == "swin_large_window12":
                pretrained_backbone_path = (
                    "../ckpts/swin_large_patch4_window12_384_22k.pth"
                )
                self.in_channels = [192, 384, 768, 1536]
            elif backbone == "swin_large_window12_to_1k":
                pretrained_backbone_path = (
                    "../ckpts/swin_large_patch4_window12_384_22kto1k.pth"
                )
                self.in_channels = [192, 384, 768, 1536]
            else:
                assert backbone == "swin_large"
                pretrained_backbone_path = (
                    "../ckpts/swin_large_patch4_window7_224_22k.pth"
                )
                self.in_channels = [192, 384, 768, 1536]
            ...
            ## initilize encoder
            state_dict = torch.load("../ckpts/" + backbone + ".pth")
    ...
    def forward(self, x):
        embeding,new_x0 = self.encoder(x)
        # print(embeding.shape)
        maskfeature=self.decoder(embeding)
        # print(embeding.shape)[4, 384, 32, 32]
        return maskfeature,new_x0
